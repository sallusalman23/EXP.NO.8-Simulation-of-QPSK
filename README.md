...
bits = np.random.randint(0, 2, num_symbols * 2) # 2 bits per QPSK
symbol
symbols = 2 * bits[0::2] + bits[1::2] # Map bits to QPSK
symbols (00, 01, 10, 11 → 0, 1, 2, 3)
# QPSK phase mapping
symbol_phases = {
0: 0,
1: np.pi / 2,
...
symbol_time = i * T
qpsk_segment = np.cos(2 * np.pi * t / T + phase) + 1j * np.sin(2 *
np.pi * t / T + phase)
...
plt.text(symbol_time + T/4, 0.5, f'{symbols[i]:02b}', fontsize=12,
color='blue')
...
plt.subplot(3, 1, 2)
plt.plot(t_total, np.imag(qpsk_signal), label='Quadrature',
color='orange')
for i, symbol_time in enumerate(symbol_times):
plt.axvline(symbol_time, color='red', linestyle='--', linewidth=0.5)
plt.text(symbol_time + T/4, 0.5, f'{symbols[i]:02b}', fontsize=12,
...
plt.subplot(3, 1, 3)
plt.plot(t_total, np.real(qpsk_signal), label='Resultant QPSK Waveform',
color='green')
for i, symbol_time in enumerate(symbol_times):
plt.axvline(symbol_time, color='red', linestyle='--', linewidth=0.5)
plt.text(symbol_time + T/4, 0.5, f'{symbols[i]:02b}', fontsize=12,
...
```
# OUTPUT
![image](https://github.com/user-attachments/assets/2c5ea694-00e8-4853-9022-e573aec2ff75)
